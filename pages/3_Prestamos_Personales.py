...
st.set_page_config(page_title="Préstamos Personales", layout="wide")
...
st.sidebar.markdown("## Filtros – Personales")
...
for campo in campos_a_usar:
    ...
    if serie.dtype == "str":
        ...
        seleccion = st.sidebar.multiselect(
            ...
            opciones,
            default=["Todas"],
            key=f"{campo}_multiselect_personales"
        )
    ...
    if pd.api.types.is_numeric_dtype(serie):
        ...
        if len(valido) == 0:
            ...
            rango = st.sidebar.slider(
                ...
                max_value=maximo,
                value=(minimo, maximo),
                key=f"{campo}_slider_personales"
            )
...
st.title("Préstamos Personales – BCRA")
...
with col_b:
    ...
    st.download_button(
        label="📥 Descargar CSV",
        data=csv_completo,
        file_name="Personales_filtrado.csv",
        mime="text/csv"
    )
