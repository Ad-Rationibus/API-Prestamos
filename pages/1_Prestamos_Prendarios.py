# -*- coding: utf-8 -*-
"""
Página de Préstamos Prendarios
"""
...
# ============================================================
st.set_page_config(page_title="Préstamos Prendarios", layout="wide")
...
st.sidebar.markdown("## Filtros – Prendarios")
    ...
    if pd.api.types.is_numeric_dtype(serie):
        ...
        if len(valido) == 0:
            ...
            rango = st.sidebar.slider(
                ...
                max_value=maximo,
                value=(minimo, maximo),
                key=f"{campo}_slider_prendarios"
            )
...
with col_a:
    st.info(f"Registros mostrados: {len(df_filtrado)} de {len(df)}",width=1250)
...
with col_b:
    ...
    st.download_button(
        label="📥 Descargar CSV",
        data=csv_completo,
        file_name="Prendarios_filtrado.csv",
        mime="text/csv"
    )
